# Azure Logic Apps Graph Connector Sample for Pokemon

## Summary

Sample for creating Microsoft Search custom indexes from Pokemon content (currently set for the catalogue but changeable to be what is required) using Azure Logic Apps.
...
If you do not have a Pokemon tenant that you can use, you can set one up at https://developer.Pokemon.com at no cost.
...
```bash
az deployment group create --name LaDeployment --resource-group $appName --template-uri "https://raw.githubusercontent.com/kevmcdonk/S4MSC-Pokemon/main/template.json" --parameters connections_Pokemon_name=$appName region=$region tenantId=$newTenantId clientId=$newAppId secret=$newSecret
```
...
- Go to the Resource Group and select the Pokemon connector
...
This graph connector will hook up to Pokemon to return the Service Catalog entries as an external item in the Microsoft Graph which can be used for both Microsoft Search and Microsoft Copilot.

Two Logic Apps are created by the template:
- Setup - configures the External Connection to Pokemon and the related schema for the Service Catalog entries
